#!/bin/bash
#Chexk system for popular package managers
#
#
########### Introuction ##############
echo "####################################"
echo 
echo "This script check your Linux system for popular"
echo "What's available, and makes an educated guess on your"
echo "distribution's base distro (Red hat or Debian)"
echo
echo "####################################"
#
############## For Redhat Check ###############
echo
echo "Checking for Red hat-based package manager &"
echo "application containers...."
####
if (which rpm &> /dev/null)
then    
    item_rpm=1
    echo "You have the basic rpm utility."
#
else
    item_rpm=0
#
fi
####
if (which dnf &> /dev/null)
then
    item_dnfyum=1
    echo "You have the dnf package manager."
elif (which yum &> /dev/null)
then
    item_dnfyum=1
    echo "You have the yum package manager."
else
    item_dnfyum=0
#
fi
####
if (which flatpak &> /dev/null)
then
    item_flatpak=1
    echo "You have the flatpak application container."
else
    item_flatpak=0
fi
####
redhatscore=$[$item_rpm + $item_dnfyum + $item_flatpak]
#
################## Debian Checks #####################
#
echo
echo "Checking for Debian based package manager &"
echo "application containers....."
####
if (which dpgk &> /dev/null)
then
    item_dgpk=1
    echo "You have the basic dgpk utiluty."
#
else
    item_dgpk=0
#
fi
####
if (which apt &> /dev/null)
then    
    item_aptaptget=1
    echo "You have the apt-get/apt-cache package manager."
elif (which apt-get &> /dev/null)
then 
    item_aptaptget=1
    echo "You have the apt-get/apt-cache package manager."
else
    item_aptaptget=0
fi
####
if (which snap &> /dev/null)
then    
    item_snap=1
    echo "You have the snap application container."
else
    item_snap=0
fi
####

debianscore=$[$item_dgpk + $item_aptaptget + $item_snap]
#
########## Detemine Distro ##################
#
echo
if [ $debianscore -gt $redhatscore ]
then
    echo "Most likely you linux Distribution is Debian-based."
elif [ $redhatscore -gt $debianscore ]
then
    echo "Most likely ypu linux distribution is Redhat-based."
else
    echo "Unable to determine Linux Distribution base.please chek"
fi
echo
#################################

